...
def check_goods_item_list_not_empty(response):
    assert len(response['Body']['GoodsItemList'][0]['Brand']) != 0


def test_send_request_goods_item_search():
    url = uri + '/GoodsItemSearchByid'
    payload['Body']['id'] = "3079071"

    response = requests.post(url, data=json.dumps(payload, indent=4), headers=headers)
    debug_save_to_json_request_and_response(payload=payload, response=response, test_purpose='проверка наличия')
